...
A Helm chart for deploying Bizon Platform Lite on Kubernetes.
...
# Access the UI
kubectl port-forward svc/bizon-bizon-platform-lite-ui 3000:8080
...
# Upgrade
helm upgrade bizon oci://ghcr.io/bizon-data/charts/bizon-platform-lite

# Pull chart locally
helm pull oci://ghcr.io/bizon-data/charts/bizon-platform-lite --version 0.1.0 --untar
```
...
docker build --target backend -t bizon-platform-lite:local .
docker build --target ui -t bizon-platform-lite-ui:local .

# Load images into minikube
docker save bizon-platform-lite:local | minikube image load --daemon=false -
docker save bizon-platform-lite-ui:local | minikube image load --daemon=false -

# Install with local images
helm install bizon ./helm/bizon-platform-lite \
  --set api.image.repository=bizon-platform-lite \
  --set api.image.tag=local \
  --set api.image.pullPolicy=Never \
  --set worker.image.repository=bizon-platform-lite \
  --set worker.image.tag=local \
  --set worker.image.pullPolicy=Never \
  --set ui.image.repository=bizon-platform-lite-ui \
...
| `postgresql.auth.database` | Database name | `bizon_platform_lite` |
...
| `externalDatabase.database` | Database name | `bizon_platform_lite` |
...
| Parameter | Description | Default |
|-----------|-------------|---------|
| `api.image.repository` | API image | `ghcr.io/bizon-data/bizon-platform-lite` |
...
| Parameter | Description | Default |
|-----------|-------------|---------|
| `worker.image.repository` | Worker image | `ghcr.io/bizon-data/bizon-platform-lite` |
...
| Parameter | Description | Default |
|-----------|-------------|---------|
| `ui.image.repository` | UI image | `ghcr.io/bizon-data/bizon-platform-lite-ui` |
...
| Image | Description |
|-------|-------------|
| `ghcr.io/bizon-data/bizon-platform-lite` | API + Worker (Python backend) |
| `ghcr.io/bizon-data/bizon-platform-lite-ui` | UI (nginx serving React) |
